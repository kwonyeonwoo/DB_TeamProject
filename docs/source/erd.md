...
| `status` | VARCHAR(20) | default `ACTIVE`, allowed `ACTIVE`, `INACTIVE`, `DELETED` | NN | 회원 상태 |
...
| `group_link` | VARCHAR(255) | UK | NN | 그룹 링크 |
...
- `users.status`는 `ACTIVE`, `INACTIVE`, `DELETED` 중 하나다.
