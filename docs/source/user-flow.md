...
flowchart TD
    ...
    groupManage -- "그룹 생성" --> groupCreate["그룹 생성 및 LEADER 등록"]
    groupCreate --> groupManage
    groupManage -- "그룹 링크로 가입" --> groupJoin["그룹 가입"]
...
| `S-11` | 게시글 상세 페이지 | 게시판 목록 또는 알림 | 조회수 증가, 추천, 댓글/대댓글, 작성자 표시, 수정/삭제 분기 |
...
| 사용자 액션 | 회원 정보 수정 페이지로 이동해 이름, 비밀번호, 이메일 중 하나 이상을 변경한다. |
| 시스템 응답 | 기존 정보와 동일한 내용인지 확인하고, 이메일 변경 시 중복 여부를 확인한다. 변경 가능한 값이면 수정 후 마이 페이지로 이동한다. |
...
| 시작 상태 | 로그인한 사용자가 마이 페이지에 있다. |
| 사용자 액션 | 회원 탈퇴를 실행한다. |
| 시스템 응답 | 회원 row는 유지하고 `deleted_at`을 기록하며 `status`를 `DELETED`로 변경한다. 개인정보성 컬럼은 삭제하거나 식별할 수 없는 값으로 변경한다. |
| 종료 상태 | 사용자는 즉시 로그아웃 처리되고 로그인 페이지로 이동한다. |
| 예외/분기 | 탈퇴 회원의 게시글, 댓글, 대댓글은 삭제하지 않고 작성자명을 `탈퇴한 유저`로 표시한다. |
| 예외/분기 | 탈퇴 회원의 개인 캘린더와 개인 일정은 `status`, `deleted_at` 기반으로 비활성화 및 삭제 대기 상태가 되며 일반 사용 흐름에서는 노출하지 않는다. |
| 예외/분기 | 탈퇴 시점에 해당 회원은 가입되어 있던 모든 그룹에서 탈퇴 처리된다. |
| 예외/분기 | 탈퇴 회원이 그룹장인 그룹은 탈퇴 처리 전에 가장 먼저 가입한 다른 그룹원에게 LEADER 권한을 위임한다. 유일한 그룹원이면 해당 그룹을 `status`, `deleted_at` 기반 비활성화 및 삭제 대기 상태로 전환하고 일반 사용 흐름에서는 노출하지 않는다. |
...
| 시스템 응답 | 게시글을 작성자의 `user_id`와 함께 저장하고, 첨부파일은 사용자가 직접 업로드한 파일로 처리한다. |
...
| 시스템 응답 | 댓글을 작성자의 `user_id`와 게시글 id에 연결해 저장한다. |
...
| 시스템 응답 | 대댓글을 작성자의 `user_id`, 게시글 id, 부모 댓글 id에 연결해 저장한다. |
...
| 예외/분기 | 사용자는 날짜, 시작 시간, 종료 시간, 일정 종류, 일정 제목, 메모를 입력해 개인 일정을 등록할 수 있다. |
...
| 사용자 액션 | 그룹명을 입력해 그룹을 생성하거나 그룹 링크를 통해 그룹에 가입한다. |
| 시스템 응답 | 그룹 생성 시 그룹 id, 그룹명, 생성자, 그룹 링크를 저장하고 생성자를 `group_members`에 `LEADER`로 자동 등록한다. |
...
| 회원 탈퇴 | 마이 페이지, 로그인 페이지 | 탈퇴 처리 → 개인정보 비식별화 → 세션 무효화 → 로그인 페이지 |
...
| 그룹 생성/가입 | 그룹 관리 | 그룹 생성 → 생성자 LEADER 등록 또는 그룹 링크로 가입 |
| 그룹 일정 | 그룹 캘린더 | 그룹원 확인 → 그룹 일정 조회/등록/수정/삭제 |

## 7. 확인 필요 사항

- 그룹 가입의 구체적인 UX는 요구사항에 명확히 정의되어 있지 않다. 현재 흐름은 기존 이미지의 링크 공유 흐름과 그룹 링크 요구사항을 기준으로 표현했다.
- 일정 종류 `type`의 구체적인 값 목록은 요구사항에 명확히 정의되어 있지 않다.
- 첨부파일 업로드 후 저장 위치 또는 파일 메타데이터 관리 방식은 요구사항에 명확히 정의되어 있지 않다.
