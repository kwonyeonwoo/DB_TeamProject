...
| File | `file` | 게시글 첨부파일 저장 위치, 원본 파일명, MIME 타입. | POST-003, POST-004 |
...
- 파일은 직접 업로드 방식이다. DB에는 `/uploads/posts/{post_id}/{UUID[.extension]}` 형식의 `file_url`, 원본 파일명, MIME 타입을 저장한다.
...
- 댓글 또는 대댓글 삭제 후에도 이미 생성된 알림은 유지하며 `notification.comment_content`는 변경하지 않는다.
...
- `commented_id`는 알림 이동을 위한 nullable hint이며 댓글 삭제 cascade FK로 사용하지 않는다.
- `comment_content`는 알림 생성 시점의 댓글/대댓글 내용 스냅샷이며 원본 수정/삭제 후에도 갱신하지 않는다.
- 회원은 본인 알림만 조회할 수 있다.
