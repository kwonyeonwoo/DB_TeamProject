...
- 최종 검증: 2026-05-17 15:24 KST, `backend`에서 `.\gradlew.bat test --rerun-tasks` PASS, 102 tests, 0 failures, 0 errors, 0 skipped
...
| RW-004 | P2 | RESOLVED | Release verification | 최종 변경 묶음 정리 후 전체 테스트를 다시 실행하고 결과를 리뷰 문서에 기록했다. | 최종 상태에서 전체 테스트 재실행이 필요했다. | 2026-05-17 15:24 KST, `.\gradlew.bat test --rerun-tasks` PASS, 102 tests, 0 failures, 0 errors, 0 skipped. |
...
| RW-004 전체 테스트 재실행 | 2026-05-17 15:24 KST, `backend`에서 `.\gradlew.bat test --rerun-tasks` PASS, 102 tests, 0 failures, 0 errors, 0 skipped. |
...
1. 배포 또는 운영 설정 단계에서 `RW-001` 세션 쿠키 정책을 확정한다.
2. 프론트엔드 화면 흐름을 다루는 단계에서 `RW-003` 알림 fallback UX를 결정한다.
